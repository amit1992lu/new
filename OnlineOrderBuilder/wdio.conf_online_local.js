...
const url = require('./onlineOrderBuilderUrl')
const REGION = process.env.REGION
const ENV = process.env.ENV
let SCRIPT_RETRY = false;
let slackChannel;
let scriptName;

if (ENV === 'PROD') {
	slackChannel = process.env.SLACK_CHANNEL_ONLINE_PROD
} else {
	slackChannel = process.env.SLACK_CHANNEL_ONLINE_QA
}
...
exports.config = {
	...
	baseUrl: url.url(process.env.REGION, process.env.ENV),
	//
	...
	onPrepare: function (config, capabilities) {
		if(process.env.ENV === 'PROD' && config.suite[0] === 'smoke') {
	...
	beforeHook: function (test, context) {
		//Setting the script name to be used when taking screenshot
		scriptName = test.parent;
	},
	/**
	...
	after: async function (result, capabilities, specs) {
		if (result === 1) {
			await browser.saveScreenshot('./customReport/'+scriptName+`${new Date().getTime()}.png`);
		}
	},
	/**
